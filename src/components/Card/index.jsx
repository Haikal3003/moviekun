...
const Card = ({ data, media_type, cardRef }) => {
  const imageURL = useSelector((state) => state.movieData.imageURL);

  return (
    <div ref={cardRef} id="card" className="relative w-64 min-h-[200px] bg-transparent rounded-lg overflow-hidden mr-8 cursor-pointer transform transition-transform duration-200 ease-in-out group">
      <div>
        <div className="relative w-full h-full">
          <img src={imageURL + data?.poster_path} className="w-full h-full object-cover rounded-lg" alt={data?.title || data?.name} />
